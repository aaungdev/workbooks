  ...
  },
  {
    ...
    function displayEmployeeData(employee) {
        ...
        let fields = [
            ...
            { label: "Years at Company", value: employee.yearsAtCompany },
            { label: "Email", value: employee.email },
            { label: "WFH Address", value: employee.wfhAdress },
            { label: "Skill Set", value: employee.skillSet },
            { label: "Projects Assigned", value: employee.projectsAssignedTo.length }
        ];
        ...
        for (const field of fields) {
            ...
            cell1.innerText = field.label;

            let cell2 = row.insertCell(1);
            cell2.innerText = field.value;
        }
    }

    function displayProjects(employee) {
        projectsList.innerHTML = '';

        if (employee.projectsAssignedTo.length == 0) {
            let listItem = document.createElement('li');
            listItem.className = 'list-group-item';
            listItem.innerText = 'No projects assigned';
            projectsList.appendChild(listItem);
            return;
        }

        for (const project of employee.projectsAssignedTo) {
            let listItem = document.createElement('li');
            listItem.className = 'list-group-item';
    ...
    employeeSelect.onchange = function() {
        let selectedEmployeeId = this.value;
        let selectedEmployee = employees.find(employee => employee.id == selectedEmployeeId);
        if (selectedEmployee) {
            displayEmployeeData(selectedEmployee);
            displayProjects(selectedEmployee);
        }
    };
